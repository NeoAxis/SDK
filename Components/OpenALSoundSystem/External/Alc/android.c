...
////!!!!vladimir
JNIEnv* pJNIEnvironment = NULL;
void SetJNIEnvironmentAndJavaVM(void* pEnv, void* pJavaVM)
{
	pJNIEnvironment = (JNIEnv*)pEnv;  
	javaVM = (JavaVM*)pJavaVM;
}
...
{
	return pJNIEnvironment;////!!!!vladimir
    //JNIEnv* env = NULL;
...
static void* thread_function(void* arg)
{
    ...
    int channelConfig = device->FmtChans == DevFmtMono ? CHANNEL_CONFIGURATION_MONO : CHANNEL_CONFIGURATION_STEREO;
    int audioFormat = (device->FmtType == DevFmtByte || device->FmtType == DevFmtUByte) ? ENCODING_PCM_8BIT : ENCODING_PCM_16BIT; 
...
static ALCboolean android_reset_playback(ALCdevice *device)
{
    ...
    if (device->FmtType != DevFmtByte && device->FmtType != DevFmtUByte)
    {
