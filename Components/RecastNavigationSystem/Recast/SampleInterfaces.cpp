...
void ShowError(const char* text)
{
	...
	MessageBoxA(NULL, text, "Error", MB_OK | MB_ICONWARNING);
#elif defined(PLATFORM_MACOS)
	CFStringRef textRef = CFStringCreateWithCString(NULL, text, kCFStringEncodingUTF8);
	CFUserNotificationDisplayAlert(0, kCFUserNotificationStopAlertLevel, NULL, NULL, NULL, 
		CFSTR("Error"), textRef, CFSTR("OK"), NULL, NULL, NULL);
	CFRelease(textRef);
#elif defined(PLATFORM_ANDROID)
//!!!!!!dr
	char tempBuffer[4096];
	sprintf(tempBuffer, "Recast: Error: %s\n", text);
...
void BuildContext::doLog(const rcLogCategory category, const char* msg, const int len)
{
	...
	//	MessageBoxA(0, text, "WARNING", MB_OK);
	//else
	//	MessageBoxA(0, text, "INFO", MB_OK);
