...
bool InputGeom::loadMesh(rcContext* ctx, float* vertices, int vertexCount, int* indices, int indexCount,
	int trianglesPerChunk)
{
	...
	if (!m_mesh->load(vertices, vertexCount, indices, indexCount))
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load '%s'", "FILENAME");
		return false;
	}
	...
	if (!rcCreateChunkyTriMesh(m_mesh->getVerts(), m_mesh->getTris(), m_mesh->getTriCount(), trianglesPerChunk, m_chunkyMesh))
	{
